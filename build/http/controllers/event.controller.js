...
__export(event_controller_exports, {
  ...
  EventGetAllController: () => EventGetAllController,
  EventGetController: () => EventGetController,
  EventSearchController: () => EventSearchController,
  EventUpdateController: () => EventUpdateController
});
...
async function EventSearchController(request, reply) {
  const validate = import_zod.z.object({
    id: import_zod.z.string()
  });
  const { id } = validate.parse(request.params);
